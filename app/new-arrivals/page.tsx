...
async function getBooks(filter: Record<string, string> = {}, options: RequestInit = { cache: 'no-store' }): Promise<Book[]> {
    ...
    console.log(`Workspaceing New Arrivals Page: ${apiUrl}`);
    try {
      ...
      {newArrivals.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 xl:gap-8">
          {newArrivals.map((book) => (
            <BookCard key={book._id} book={book} badgeText="NewArrival" badgeColor="bg-bookscape-gold text-bookscape-dark" />
          ))}
        </div>
      ) : (
        <p className="text-center text-gray-700 text-lg">No new arrivals available at the moment. Check back soon!</p>
      )}
    </div>
  );
}